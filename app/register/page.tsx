...
export default function RegisterPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!registerRes.ok) { setError(registerData.error || 'Registration failed'); return }
    ...
    <div className="min-h-screen bg-white flex">
      ...
      <div className="hidden lg:flex w-1/2 bg-gray-950 flex-col justify-between p-10">
        ...
        <div className="space-y-6">
          {[
            { title: 'Free forever', desc: '10 projects and 3 keys per project on the free plan. No card required.' },
            { title: 'Secure by default', desc: 'Keys hashed with SHA-256. Shown once. Revokable instantly.' },
            { title: 'Production ready', desc: 'Rate limiting, circuit breaker, and request logging out of the box.' }
          ].map(item => (
            <div key={item.title}>
        ...
        <p className="text-gray-600 text-xs">© 2026 GateKey</p>
      </div>
